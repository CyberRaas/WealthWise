...
export class VoiceExpenseProcessor {
  constructor() {
    ...
    if (!process.env.GROQ_API_KEY) {
      console.warn('⚠️ GROQ_API_KEY not set. Voice processing will use fallback mode.')
    }
...
`
      ...
      const completion = await this.groq.chat.completions.create({
        messages: [
          { role: 'system', content: 'You are an accurate expense categorization assistant. You MUST output only valid JSON.' },
