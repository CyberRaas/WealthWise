...
class EmailService {
  ...
  /**
   * Send split group invitation to non-registered user
   */
  async sendSplitGroupInvitation({ email, groupName, invitedBy, groupId }) {
    const signupUrl = `${process.env.NEXTAUTH_URL}/auth/signin?callbackUrl=/dashboard/expenses?tab=split&groupId=${groupId}`
    const dashboardUrl = `${process.env.NEXTAUTH_URL}/dashboard/expenses?tab=split`
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>You've been added to a group on WealthWise</title>
      </head>
      <body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
          <tr>
                ...
                <tr>
                  <td style="padding: 40px;">
                    ...
                    <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 16px; line-height: 1.6;">
                      <strong>${invitedBy}</strong> has added you to a split expense group on <strong>WealthWise</strong>. 
                      You can now split expenses, track who owes what, and settle up easily!
                    </p>

                    <div style="background-color: #f0fdf4; border-left: 4px solid #10b981; padding: 16px; margin: 24px 0; border-radius: 4px;">
                      <p style="margin: 0; color: #065f46; font-size: 14px; line-height: 1.6;">
                        <strong>💡 What is Split Expenses?</strong><br>
                        Split expenses with friends, family, or roommates. Track shared costs for trips, dinners, rent, and more. 
                        WealthWise automatically calculates who owes what and simplifies settlements.
                      </p>
                    </div>
                    ...
                    <div style="margin: 32px 0;">
                      ...
                      <ul style="margin: 0; padding-left: 20px; color: #4b5563; font-size: 15px; line-height: 1.8;">
                        <li>View shared expenses in "${groupName}"</li>
                        <li>Add your own expenses to the group</li>
                        <li>See who owes you and who you owe</li>
                        <li>Settle up with easy payment tracking</li>
                        <li>Get automatic balance calculations</li>
                      </ul>
                    </div>
                    ...
                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin: 32px 0;">
                      <tr>
                        <td style="border-radius: 8px; background: linear-gradient(135deg, #10b981 0%, #059669 100%);">
                          <a href="${signupUrl}" style="display: inline-block; padding: 16px 32px; color: #ffffff; text-decoration: none; font-weight: 600; font-size: 16px;">
                            Join Group & Sign Up
                          </a>
                        </td>
                      </tr>
                    ...
                    <p style="margin: 24px 0 0 0; color: #6b7280; font-size: 14px; line-height: 1.6;">
                      Already have an account? <a href="${dashboardUrl}" style="color: #10b981; text-decoration: none; font-weight: 600;">View the group here</a>
                    </p>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="background-color: #f9fafb; padding: 24px 40px; border-top: 1px solid #e5e7eb;">
                    <p style="margin: 0 0 8px 0; font-size: 14px; color: #6b7280; text-align: center;">
                      This invitation was sent by ${invitedBy} through WealthWise
                    </p>
                    <p style="margin: 0; font-size: 12px; color: #9ca3af; text-align: center;">
                      © ${new Date().getFullYear()} WealthWise. All rights reserved.
                    </p>
    ...
    const text = `
      YOU'VE BEEN ADDED TO A SPLIT GROUP!

      ${invitedBy} has added you to "${groupName}" on WealthWise.

      What is Split Expenses?
      Split expenses with friends, family, or roommates. Track shared costs for trips, dinners, rent, and more. 
      WealthWise automatically calculates who owes what and simplifies settlements.

      What you can do:
      - View shared expenses in "${groupName}"
      - Add your own expenses to the group
      - See who owes you and who you owe
      - Settle up with easy payment tracking
      - Get automatic balance calculations

      Join the group: ${signupUrl}

      Already have an account? View the group: ${dashboardUrl}
    ...
    return await this.sendEmail({
      to: email,
      subject: `👥 ${invitedBy} added you to "${groupName}" on WealthWise`,
      html,
      text
    })
  }
  ...
  /**
   * Send notification when an expense is added to a split group
   */
  async sendExpenseAddedNotification({ email, userName, groupName, expenseDescription, amount, addedBy }) {
    const dashboardUrl = `${process.env.NEXTAUTH_URL}/dashboard/expenses?tab=split`
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>New expense in ${groupName}</title>
      </head>
      <body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
          <tr>
            <td align="center" style="padding: 40px 20px;">
              <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08);">
                ...
                <tr>
                  <td style="padding: 32px;">
                    ...
                    <p style="margin: 0 0 24px 0; color: #111827; font-size: 16px; line-height: 1.6;">
                      <strong>${addedBy}</strong> added a new expense to <strong>"${groupName}"</strong>
                    </p>

                    <div style="background-color: #f0fdf4; border: 2px solid #10b981; padding: 20px; margin: 24px 0; border-radius: 8px;">
                    ...
                    <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin: 24px 0;">
                      <tr>
                        <td style="border-radius: 8px; background: linear-gradient(135deg, #10b981 0%, #059669 100%);">
                          <a href="${dashboardUrl}" style="display: inline-block; padding: 14px 28px; color: #ffffff; text-decoration: none; font-weight: 600; font-size: 15px;">
                            View Group Details
                          </a>
                        </td>
                      </tr>
    ...
    return await this.sendEmail({
      to: email,
      subject: `💰 New expense in "${groupName}" - ₹${amount.toLocaleString('en-IN')}`,
      html,
      text
    })
  }
...
// Named export for direct sendEmail usage
