...
export const getUserFriendlyMessage = (code) => {
  return USER_FRIENDLY_MESSAGES[code] || USER_FRIENDLY_MESSAGES['INTERNAL_ERROR']
}
...
export const formatErrorResponse = (error) => {
  ...
  if (error instanceof AppError) {
    // Report server errors (500+) to Sentry
  ...
  // Default error - always report to Sentry
  Sentry.captureException(error, {
  ...
  return NextResponse.json({
    ...
    userMessage: getUserFriendlyMessage('INTERNAL_ERROR'),
    code: 'INTERNAL_ERROR',
    ...(isDevelopment && { stack: error.stack }),
  }, { status: 500 })
}
