    ...
    try {
      ...
      if (splitType !== 'equal' && Object.keys(customSplits).length > 0) {
        expenseData.splits = Object.entries(customSplits).map(([userId, value]) => ({
          ...
          percentage: splitType === 'percentage' ? parseFloat(value) : undefined
        }))
      }
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-2">
              {EXPENSE_CATEGORIES.map(cat => (
                ...
                >
                  ...
                  <p className="text-xs mt-1 truncate text-slate-900 dark:text-slate-100">{cat.name}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Paid By */}
          <div className="space-y-2">
            <Label className="text-slate-700 dark:text-slate-300">Paid by *</Label>
              ...
              {members.map(member => (
                ...
                >
                  <Avatar className="h-8 w-8">
                    ...
                    <AvatarFallback className="bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300">{getInitials(member.user.name)}</AvatarFallback>
                  </Avatar>
                  <span className="text-sm font-medium truncate text-slate-900 dark:text-slate-100">{member.user.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Split Type */}
          <div className="space-y-2">
            <Label className="text-slate-700 dark:text-slate-300">Split Type</Label>
            <RadioGroup value={splitType} onValueChange={setSplitType} className="grid grid-cols-3 gap-2">
              <label className={`flex items-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                splitType === 'equal' ? 'border-emerald-500 bg-emerald-500/10 dark:bg-emerald-500/20' : 'border-slate-300 dark:border-slate-600'
              }`}>
                <RadioGroupItem value="equal" className="sr-only" />
                <Equal className="h-4 w-4 text-slate-700 dark:text-slate-300" />
                <span className="text-sm text-slate-900 dark:text-slate-100">Equal</span>
              </label>
              <label className={`flex items-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                splitType === 'exact' ? 'border-emerald-500 bg-emerald-500/10 dark:bg-emerald-500/20' : 'border-slate-300 dark:border-slate-600'
              }`}>
                <RadioGroupItem value="exact" className="sr-only" />
                <Calculator className="h-4 w-4 text-slate-700 dark:text-slate-300" />
                <span className="text-sm text-slate-900 dark:text-slate-100">Exact</span>
              </label>
              <label className={`flex items-center gap-2 p-3 rounded-lg border-2 cursor-pointer transition-all ${
                splitType === 'percentage' ? 'border-emerald-500 bg-emerald-500/10 dark:bg-emerald-500/20' : 'border-slate-300 dark:border-slate-600'
              }`}>
                <RadioGroupItem value="percentage" className="sr-only" />
                <Percent className="h-4 w-4 text-slate-700 dark:text-slate-300" />
                <span className="text-sm text-slate-900 dark:text-slate-100">Percent</span>
              </label>
            </RadioGroup>
                ...
                {members.map(member => (
                  <div key={member.user._id} className="flex items-center gap-3">
                    <Avatar className="h-6 w-6">
                      <AvatarFallback className="text-xs bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300">{getInitials(member.user.name)}</AvatarFallback>
                    </Avatar>
                    <span className="text-sm flex-1 text-slate-900 dark:text-slate-100">{member.user.name}</span>
                    <Input
                      type="number"
                      value={customSplits[member.user._id] || ''}
                      onChange={(e) => handleSplitChange(member.user._id, e.target.value)}
                      placeholder="0"
