...
function LoanAdvisorChat({ calculationResults }) {
    ...
    const [messages, setMessages] = useState([{
        id: 1,
        type: 'bot',
        content: t('debtCalculator.chatGreeting'),
        timestamp: new Date()
    }])
    ...
    useEffect(() => {
        setMessages(prev => {
            ...
            if (newMessages[0].id === 1) {
                newMessages[0].content = t('debtCalculator.chatGreeting')
            }
            return newMessages
        })
    }, [t])
    ...
    const quickActions = [
        t('debtCalculator.quickAction.calculate'),
        t('debtCalculator.quickAction.strategy'),
        t('debtCalculator.quickAction.creditScore'),
        t('debtCalculator.quickAction.banks')
    ]
